...
public:
    ...
    {
        FILE *f = _wfopen(sFile.c_str(), L"wb"); // MinGW fix
        if (f == nullptr)
            return false;
    ...
    bool Load(std::wstring sFile)
    {
        ...
        FILE *f = _wfopen(sFile.c_str(), L"rb"); // MinGW fix
        if (f == nullptr)
            return false;
    ...
    void GameThread()
    {
        ...
        while (m_bAtomActive)
        {
            while (m_bAtomActive)
            {
                ...
                wchar_t s[256];
                swprintf(s, 256, L"OneLoneCoder.com - Console Game Engine - %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime); // MinGW fix
                SetConsoleTitle(s);
...
protected: // Audio Engine =====================================================================
    ...
    public:
        ...
        {
            FILE *f = _wfopen(sWavFile.c_str(), L"rb"); // MinGW fix
            if (f == nullptr)
                return;
